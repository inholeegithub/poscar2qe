...
# Written by In-Ho Lee, KRISS, May 3, 2020.
def extcell(n1,n2,n3,natot,a1,a2,a3,drt):
    uec=np.zeros(3)
    vec=np.zeros(3)
    wec=np.zeros(3)
    cmt=np.zeros((3,3))
    natomsext=natot*(n1*n2*n3)
    carext=np.zeros((natomsext,3))
    drtext=np.zeros((natomsext,3))
    natomsext=0
    for i in range(n1):
        for j in range(n2):
            for k in range(n3):
                uec[0]=i*a1[0]+j*a2[0]+k*a3[0]
                uec[1]=i*a1[1]+j*a2[1]+k*a3[1]
                uec[2]=i*a1[2]+j*a2[2]+k*a3[2]
                for iatom in range(natot):
                    vec[0]=drt[iatom,0]*a1[0]+drt[iatom,1]*a2[0]+drt[iatom,2]*a3[0]
                    vec[1]=drt[iatom,0]*a1[1]+drt[iatom,1]*a2[1]+drt[iatom,2]*a3[1]
                    vec[2]=drt[iatom,0]*a1[2]+drt[iatom,1]*a2[2]+drt[iatom,2]*a3[2]
                    wec[:]=vec[:]+uec[:]
                    carext[natomsext,:]=wec[:]
                    natomsext=natomsext+1
    cmt[0,:]=n1*a1[:] 
    cmt[1,:]=n2*a2[:] 
    cmt[2,:]=n3*a3[:]
    vtsty=(cmt[0,1]*cmt[1,2]-cmt[0,2]*cmt[1,1])*cmt[2,0] 
    vtsty=vtsty+(cmt[0,2]*cmt[1,0]-cmt[0,0]*cmt[1,2])*cmt[2,1] 
    vtsty=vtsty+(cmt[0,0]*cmt[1,1]-cmt[0,1]*cmt[1,0])*cmt[2,2]
    vtsty=abs(vtsty)
    aa1=n1*a1
    aa2=n2*a2
    aa3=n3*a3
    ...
    if False:
       ...
       print(b-invmat)
    for iatom in range(natomsext):
        drtext[iatom,:]=cart2drt(b,carext[iatom,:])
    if False:
       for iatom in range(natomsext):
           drtext[iatom,:]=cart2drt(b,carext[iatom,:])
           x=drtext[iatom,0]*aa1[0]+drtext[iatom,1]*aa2[0]+drtext[iatom,2]*aa3[0]
           y=drtext[iatom,0]*aa1[1]+drtext[iatom,1]*aa2[1]+drtext[iatom,2]*aa3[1]
           z=drtext[iatom,0]*aa1[2]+drtext[iatom,1]*aa2[2]+drtext[iatom,2]*aa3[2]
           print(x-carext[iatom,0],y-carext[iatom,1],z-carext[iatom,2])
    return natomsext,aa1,aa2,aa3,drtext
# Written by In-Ho Lee, KRISS, May 3, 2020.
ipseudo=2
afile=open('POSCAR','r')
line=afile.readline()
...
if scale0 < 0.:
   ...
   cmatrix[1,:]=a2[:]
   cmatrix[2,:]=a3[:]
   vtest=(cmatrix[0,1]*cmatrix[1,2]-cmatrix[0,2]*cmatrix[1,1])*cmatrix[2,0]
   vtest=vtest+(cmatrix[0,2]*cmatrix[1,0]-cmatrix[0,0]*cmatrix[1,2])*cmatrix[2,1]
   vtest=vtest+(cmatrix[0,0]*cmatrix[1,1]-cmatrix[0,1]*cmatrix[1,0])*cmatrix[2,2]
   vtest=(abs(scale0)/abs(vtest))**(1.0/3.0)
...
for i in range(nspecies):
    symbl.append(line.split()[i])
#   print(line.split()[i])
nelements=[]
line=afile.readline()
for i in range(nspecies):
    nelements.append(int(line.split()[i]))
#   print(line.split()[i])
line=afile.readline()
if line.split()[0].lower() == 'selective' :
   line=afile.readline()
natot=0
for i in range(nspecies):
    natot=natot+nelements[i]
drt=np.zeros((natot,3))
isymbl=[] 
for i in range(nspecies):
    for k in range(nelements[i]):
        isymbl.append(symbl[i])
line6=line.split()[0]
if line6[0] == 'd' or line6[0] == 'D' :
   line6='direct'
if line6 == 'direct' :
   for k in range(natot):
       line=afile.readline()
       drt[k,0]=float(line.split()[0])
       drt[k,1]=float(line.split()[1])
       drt[k,2]=float(line.split()[2])
else:
   for k in range(natot):
       line=afile.readline()
       drt[k,0]=float(line.split()[0])
...
if True:
   ...
   bfile.write('conv_thr= 1.0d-7'+  '\n')
   bfile.write('mixing_beta=0.7'+  '\n')
   bfile.write('/'+  '\n')
   bfile.write(''+  '\n')
